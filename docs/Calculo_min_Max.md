...
Este script é executado pelo módulo de Análises do TagoIO, configurado para rodar automaticamente em um intervalo periódico. Desenvolvido em Python, ele lê os dados do dispositivo indicado via device_token e calcula os valores mínimo, máximo e médio de uma variável de temperatura nas últimas 24 horas, registrando cada resultado como novas variáveis no bucket do próprio dispositivo.
...
def temperature_minimum(device: Device) -> None:
    ...
    min_filter = {"variables": "temp_femeas", "query": "min", "start_date": "1 day"}
